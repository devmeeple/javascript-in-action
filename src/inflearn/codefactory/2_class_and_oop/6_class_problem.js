...
const cIveMembers = iveMembers.map(
  (value) => new FemaleIdol(value.name, value.year),
);
console.log(cIveMembers);

const cBtsMembers = btsMembers.map(
  (value) => new MaleIdol(value.name, value.year),
);
...
const allTogether = new Country('대한민국', [
  new IdolGroup(
    '아이브',
    iveMembers.map((value) => new FemaleIdol(value.name, value.year)),
  ),
  new IdolGroup(
    '방탄소년단',
    btsMembers.map((value) => new MaleIdol(value.name, value.year)),
  ),
]);
